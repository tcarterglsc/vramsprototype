...
export default function EditRequestPanel({ requestId, onClose }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-white">
        ...
        <button type="button" onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 text-lg leading-none">×</button>
      </div>
            ...
            render={({ field }) => (
              <div className="flex rounded-lg border border-gray-200 overflow-hidden">
                {(['fixed', 'flexible'] as const).map((opt) => (
                  <button key={opt} type="button" onClick={() => field.onChange(opt)} className={`flex-1 py-2 text-sm font-semibold transition-colors ${field.value === opt ? 'bg-blue-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}>
            ...
            render={({ field }) => (
              <div className="flex gap-2">
                {([
                  ...
                  <button key={opt.value} type="button" onClick={() => field.onChange(opt.value)} className={`flex-1 py-1.5 text-xs font-semibold rounded-lg border transition-all ${field.value === opt.value ? opt.cls : 'border-gray-200 bg-white text-gray-400 hover:border-gray-300'}`}>
                    {opt.label}
                  </button>
                ))}
              </div>
      ...
      <div className="border-t border-slate-200 bg-white p-4 flex gap-3">
        <button type="button" onClick={onClose} disabled={saving} className="flex-1 py-2.5 border border-gray-300 text-sm font-semibold text-gray-700 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50">Cancel</button>
        <button type="button" onClick={handleSubmit(onSubmit)} disabled={saving} className="flex-1 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
